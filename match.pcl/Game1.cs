...
namespace match.pcl
{
	...
	public class Game1 : Game
	{
		...
		protected override void LoadContent()
		{
			...
			base.LoadContent();
			using (var stream = TitleContainer.OpenStream("Content/cursor2.png"))
			{
				cursorTex = Texture2D.FromStream(GraphicsDevice, stream);
			}
			using (var stream = TitleContainer.OpenStream("Content/background.png"))
			{
				background = Texture2D.FromStream(GraphicsDevice, stream);
			}
			mainMenu = new Texture2D(GraphicsDevice, 1, 1);
			mainMenu.SetData(new[] { Color.White });
			...
			{
				buttonStartString = Texture2D.FromStream(GraphicsDevice, stream);
			}
			using (var stream = TitleContainer.OpenStream("Content/again.png"))
			{
				buttonTextureStartNewGame = Texture2D.FromStream(GraphicsDevice, stream);
			}
			buttonTextureStartNewGameHover = buttonTextureStartNewGame;

			for (var _ = 1; _ < 8; _++)
				using (var stream = TitleContainer.OpenStream($"Content/0{_}.png"))
				{
					TexturesElements.Add(Texture2D.FromStream(GraphicsDevice, stream));
				}


			using (var stream = TitleContainer.OpenStream("Content/Sun.png"))
			{
				meteor = Texture2D.FromStream(GraphicsDevice, stream);
				TexturesElements.Add(meteor);
			}

			using (var stream = TitleContainer.OpenStream("Content/bucket.png"))
			{
				TexturesElements.Add(Texture2D.FromStream(GraphicsDevice, stream));
			}
		}
		...
		protected override void Update(GameTime gameTime)
		{
			...
			if (mouseState.LeftButton != ButtonState.Released || lastMouseState.LeftButton != ButtonState.Pressed) return;
			Debug.WriteLine($"LMB was pressed in ({mouseState.X},{mouseState.Y})");
				...
				foreach (var e in GameProcess.ListElements.ToArray())
				{
					foreach (var u in e.ToArray())
					{
						spriteBatch.Draw(
							...
							,
							new Rectangle(u.getXcoord(), u.getYcoord(), (int) Math.Round((WIDTH - 20)*u.Rate),
								(int) Math.Round((WIDTH - 20)*u.Rate)) //dest rectangle
							, null
							, Color.White*u.Opacity
							, u.TextureRotationAngle() //rotation
							, new Vector2((WIDTH - 20)*u.XTextureRotationMultiplier, (WIDTH - 20)*u.YTextureRotationMultiplier)
							//new Vector2 (0, 0)//, 
							, SpriteEffects.None
							, 1
						);
